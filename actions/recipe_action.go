...
func (recipe *RecipeAction) Verify(context *debos.DebosContext) error {
	...
	for k, v := range recipe.Variables {
		/* By default included recipe shared the same arhitecture with its
		   parent. Overwrite the local context "architecture" in case this
		   is set explicitly in Variables options */
		if k == "architecture" {
